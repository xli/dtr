...
Start DTR agent with providing 1 runner in group 'mysql-firefox' as follows:
  ...
  dtr --broadcast_ip broadcast_ip -r runner1 -g mysql-firefox
...
DTR broadcast_list and group configuration would be cached in the directory running the test or agent process.
Name of configuration file is '.dtr_env_pstore'.
...
=== Options are:
  ...
  -a, --broadcast_address ADDRESS  Specify broadcast address for looking up dtr service, e.g. 192.168.255.255. Default is 'localhost'.
